...
public class PrEffectHelper {
    ...
    static Rectangle clipbounds(Point2D cul, Point2D cur, Point2D cll, Point2D clr) {
        ...
        if (cul != null && cur != null && cll != null && clr != null) {
            ...
            if (cll.x < clr.x) {
                x1 = Math.min(x1, cll.x);
                x2 = Math.max(x2, clr.x);
            } else {
                x1 = Math.min(x1, clr.x);
                x2 = Math.max(x2, cll.x);
            }
            if (cll.y < clr.y) {
                y1 = Math.min(y1, cll.y);
                y2 = Math.max(y2, clr.y);
            } else {
                y1 = Math.min(y1, clr.y);
                y2 = Math.max(y2, cll.y);
            }
            // See FilterEffect.untransformClip for a description of
